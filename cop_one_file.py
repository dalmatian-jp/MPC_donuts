...
columns_to_convert = [
    ...
    'u1_s', 'u2_s','q1_l_est','q2_l_est','dq1_l_est','dq2_l_est','q1_l_true','q2_l_true','dq1_l_true','dq2_l_true','u1_l','u2_l'
]

# データの読み込み
data = pd.read_csv("data/AllData_takami_60s.csv")
...
q1_l = data['q1_s_true'].values
q2_l = data['q2_s_true'].values
dq1_l = data['dq1_s_true'].values
dq2_l = data['dq2_s_true'].values
u1_l = data['u1_s'].values
u2_l = data['u2_s'].values


# COPを格納する配列
num_steps = len(data)
COP_s_values = np.zeros(num_steps)
COP_l_values = np.zeros(num_steps)
...
data['COP_l'] = COP_l_values


# CSVファイルに保存
data.to_csv("data/AllData_takami_60s_with_COP.csv", index=False)
